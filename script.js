...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function showContactMenu(formData) {
        const overlay = document.getElementById('contact-menu-overlay');
        if (overlay) {
            overlay.classList.add('show');
            // Store form data globally for use in handlers
    ...
    function generateWhatsAppMessage(data) {
        const message = `🎓 *Study Abroad Inquiry* 🎓\n\n` +
            `*Name:* ${data.name}\n` +
            `*Phone:* ${data.phone}\n` +
            `*Email:* ${data.email}\n` +
            `*Target Country:* ${data.country}\n` +
            `*Message:* ${data.message || 'Looking for study abroad guidance'}\n\n` +
            `Sent via TEG Website`;
        
        return encodeURIComponent(message);
    }

    function generateEmailSubject(data) {
        return `Study Abroad Inquiry - ${data.name} - ${data.country}`;
    }

    function generateEmailBody(data) {
        return `Hello TEG Team,\n\n` +
            `I would like to inquire about studying abroad. Here are my details:\n\n` +
            `Name: ${data.name}\n` +
            `Phone: ${data.phone}\n` +
            `Email: ${data.email}\n` +
            `Target Country: ${data.country}\n` +
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', function(e) {
            try {
                ...
                // Basic validation
                if (!data.name || !data.phone || !data.email || !data.country) {
                ...
                // Show contact menu instead of direct submission
                showContactMenu(data);
            } catch (error) {
        ...
        whatsappOption.addEventListener('click', function(e) {
            try {
                ...
                if (data) {
                    ...
                    const whatsappUrl = `https://wa.me/919700500087?text=${message}`;
                    window.open(whatsappUrl, '_blank');
                    hideContactMenu();
                }
            } catch (error) {
                console.error('WhatsApp option error:', error);
            }
        });
    }
    ...
    if (emailOption) {
        emailOption.addEventListener('click', function(e) {
            try {
                ...
                const data = window.contactFormData;
                if (data) {
                    const subject = encodeURIComponent(generateEmailSubject(data));
                    const body = encodeURIComponent(generateEmailBody(data));
                    const emailUrl = `mailto:pritpal@theeducationgroup.net?subject=${subject}&body=${body}`;
                    window.location.href = emailUrl;
                    hideContactMenu();
                }
            } catch (error) {
                console.error('Email option error:', error);
            }
        });
    }

    // Close menu when clicking overlay
    const contactMenuOverlay = document.getElementById('contact-menu-overlay');
    if (contactMenuOverlay) {
        contactMenuOverlay.addEventListener('click', function(e) {
            try {
                if (e.target === this) {
                    hideContactMenu();
                }
            } catch (error) {
                console.error('Contact menu overlay click error:', error);
            }
        });
